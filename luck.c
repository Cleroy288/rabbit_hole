...
char	*ft_dispatch(va_list *lst, char *string, char c)
{
	...
	/*else if (c == 'f')
		return (string = ft_itoa((va_arg(*lst, double)), "0123456789"));*/
	else if (c == 'o')
		return (string = ft_itoa((va_arg(*lst, int)), "01234567"));
	return (string = ft_str("nop"));
}
//////////////////////////////////////////////
...
char	**ft_printf_2(char const *str, va_list ag)
{
	...
	while (str[i])
	{
		if (ft_numb_of_strings(str) != 0)
		{
			if (str[i] == '%' && ++j <= ft_numb_of_strings(str) && ft_is_in(str[i + 1], "ocspiudxX%") == 1)
			{
...
int	ft_printf(char const *str, ...)
{
	...
	{		
		if (str[i] == '%' && ft_is_in(str[i + 1], "ocsipudxX%") == 1 && ++j < ft_numb_of_strings(str) && ++i)
